...
<body>
<main class="shell">
<section class="hero"><span class="badge">Dorm Demo</span><h1>留言板 #__PORT__</h1><p class="subtitle">这是一个会在进程启动时随机换背景色的临时留言板。留言只保存在内存里，刷新可见，重启即失。</p></section>
...
def page_bytes(port):
    return PAGE.replace("__PORT__", str(port)).replace("__BG__", BACKGROUND).replace("__ACCENT__", ACCENT).encode("utf-8")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path == "/":
            body = page_bytes(self.server.server_port)
...
if __name__ == "__main__":
    ...
    print(f"留言板已启动: http://localhost:{port} (背景色: {BACKGROUND})")
